...
import { LOCAL_AGENT_PROVIDERS } from '../../kuundaAi/common/agentProviders.js';

export const STRICT_OFFLINE_DEFAULT = false;

export const EXTERNAL_FEATURES = ['llm_cloud', 'kuunda_cloud', 'publish', 'billing', 'credits', 'auth', 'feedback', 'usage_telemetry'] as const;
...
export const PAYMENT_AGGREGATORS = [
	{ id: 'genius-pay', status: 'current', regions: ['XOF'], kind: 'mobile_money_psp' },
] as const;
...
export const CREDIT_REFUND_POLICY = {
	unusedPurchased: 'request',
	consumed: 'non_refundable',
	storeIapIfUsed: 'store_rules',
} as const;

export function isLocalAgentProvider(name: string | undefined): boolean {
	return (LOCAL_AGENT_PROVIDERS as readonly string[]).includes(String(name || ''));
}

export function isKnownPaymentAggregator(id: string): boolean {
	return PAYMENT_AGGREGATORS.some((row) => row.id === id);
}

export function resolvePaymentAggregator(id?: string): string {
...
export function decideExternalSend(input: {
	...
	feature?: string;
	provider?: string;
} = {}): { ok: true; feature: string; localOnly?: boolean } | { ok: false; error: 'strict_offline' | 'silent_telemetry_forbidden'; disablesBilling?: true; feature: string } {
	...
	if ((EXTERNAL_FEATURES as readonly string[]).includes(feature)) {
		return { ok: false, error: 'strict_offline', disablesBilling: true, feature };
	}
	return { ok: true, feature };
}

export function formatDataDisclosure(input: { locale?: string; strictOffline?: boolean; aggregatorId?: string } = {}): string {
	const fr = typeof input.locale === 'string' && (input.locale === 'fr' || input.locale.startsWith('fr'));
	const aggregator = resolvePaymentAggregator(input.aggregatorId);
	const lines = fr
		? [
...
export function formatRefundPolicy(locale = 'en'): string {
...
export function describeLicenseSplit(): {
	...
	inheritedEditorCore: string;
	proprietaryRepo: string;
	proprietaryModules: string[];
	paymentAggregatorsPluggable: boolean;
} {
	return {
...
export function formatDataSubjectRights(locale = 'en'): string {
