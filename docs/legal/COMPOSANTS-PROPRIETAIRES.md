...
1. **Apache-2.0 §4, dernier alinéa** permet des termes différents pour *ses propres* modifications, à condition de rester conforme pour le Work d'origine. Si la logique Genius Pay / crédits est commitée dans le même arbre sous `LICENSE` Apache-2.0, elle devient redistribuable sous Apache-2.0.
...
Ces artefacts sont Apache-2.0. Ils ne révèlent pas la logique anti-fraude, les webhooks Genius Pay, ni les credentials opérateur.
...
| `genius-pay` | Vérification de signature des webhooks, rapprochement Mobile Money |
...
- Aucune clé API, identifiant Google Play / Apple, credential Kuunda Cloud, secret Genius Pay, ou donnée de paiement en clair dans un fichier versionné, **y compris en local si ce fichier peut être commité**.
...
- Phase 3bis (crédits) : UI + interface dans le dépôt public ; ledger et Genius Pay dans le dépôt privé.
